...
[![Backend Tests](https://github.com/footom/kitchen_detection/actions/workflows/backend-tests.yml/badge.svg)](https://github.com/footom/kitchen_detection/actions/workflows/backend-tests.yml)
[![Android Tests](https://github.com/footom/kitchen_detection/actions/workflows/android-tests.yml/badge.svg)](https://github.com/footom/kitchen_detection/actions/workflows/android-tests.yml)
...
| 測試檔 | 類型 | 涵蓋內容 |
|---|---|---|
| `backend/lib/sensorLogic.test.js` | 單元測試 | 資料驗證、時間格式化、門檻判斷、警報冷卻機制 |
| `backend/test/api.test.js` | 整合測試 | 六支REST API（含正常流程與錯誤處理），mysql2/mqtt皆為mock |
...
| 測試檔 | 類型 | 涵蓋內容 |
|---|---|---|
| `app/src/test/.../AlertThresholdsTest.java` | 單元測試 | 本機端 LPG/CO 警報門檻判斷 |
| `app/src/test/.../HistoryUrlBuilderTest.java` | 單元測試 | 歷史查詢 API 網址組合邏輯 |

執行方式（不需開啟 Android Studio）：
...
測試報告：`android/app/build/reports/tests/testDebugUnitTest/index.html`

### CI

`main`分支上的push與PR會由GitHub Actions自動執行對應的測試，設定檔在`.github/workflows/`：
* `backend-tests.yml`：`backend/`有變動時執行
* `android-tests.yml`：`android/`有變動時執行

目前尚未涵蓋：ESP8266韌體、Android需要模擬器的Instrumented Test（登入畫面、圖表互動等）。
